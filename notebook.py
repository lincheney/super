...
def parse_aussuper(aussuper_raw, datetime, mo):
    ...
    for _row in aussuper_raw:
        ...
        if not _year.isdigit():
            continue
        _date = datetime.datetime(int(_year), 6, 30)
...
@app.cell
def make_alldata(aussuper, datetime, ending_balance, hostplus, itertools):
    alldata = sorted(hostplus + aussuper, key=lambda x: (x['name'], -x['date'].timestamp()))
...
def filter_rev_cumproduct(aussuper, hostplus, mo):
    ending_balance = mo.ui.number(start=100_000, label="Ending balance")
